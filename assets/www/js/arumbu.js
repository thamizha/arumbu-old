...
/*
	Name Object - Handle data as objects
*/
function NameObj() {
	this.id = "";
	this.name = "";	
...
/*
	...
	We have three tabs named -  all, male and female
	Three tabs common functionality can be set in this class.	
*/
...
var arumbu = {		
	...
	"hideNameList":function(arg) {
		...
		if($(sel).length>0) {
			$(sel).animate({'opacity':0},callback);
		}
	},
	
	"showNameList":function(callback) {
		if($('.ui-page-active .nameList').length>0) {
				$('.ui-page-active .nameList').animate({'opacity':1},callback);
		}
	},	
...
var arumbuDb = {		
		...
		"query" :function(qry,callback) {
			...
			function queryDb(tx) {
				if(tx) {				
					tx.executeSql(qry,[],callback,_parent.dbError);
				}
			}			
			
			this.dbObj.transaction(queryDb);									
		},
		
		"prepare":function(tx) {		
			if(tx) {			
				tx.executeSql("DROP TABLE IF EXISTS names");
				tx.executeSql("DROP TABLE IF EXISTS starred");
				tx.executeSql("CREATE TABLE IF NOT EXISTS names (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, meaning TEXT, gender TEXT, prefix TEXT)");
				tx.executeSql("CREATE TABLE IF NOT EXISTS starred (id INTEGER PRIMARY KEY AUTOINCREMENT, name_id INTEGER)");				
			}else {				
				this.dbObj.transaction(this.prepare,this.dbError);
			}			
		},	
		...
		"initialize":function() {
			...
			if(window.localStorage.getItem("isArumbuDbInitializes")!="yes") {
				this.prepare();
				this.initializeData();
			}
		...
		"initializeData":function(tx) {
			if(tx) {
				for(var i=0;i<arumbuDb.data.length;i++) {
					tx.executeSql(arumbuDb.data[i]);
				}
				window.localStorage.setItem("isArumbuDbInitializes", "yes");
			}else {
				if(arumbuDb.dbObj) {
					this.dbObj.transaction(this.initializeData);
				}
			}
		},
