...
ListTab.prototype.gotoPage = function (pageNo) {
		if( ! (pageNo<0 && pageNo> this.totalPages)) {
			...
			this.dispatchEvent(this.events.PAGECHANGED,this.currentPage);
		}
}
...
var arumbu = {		
	...
	"init":function() {
		//function initialize
		 $(document).ready(function(){                   
			...
			var op1="<ul data-role='listview' class='nameList'>";
			var op2="<ul data-role='listview' class='nameList'>";
			var op3="<ul data-role='listview' class='nameList'>";
			
			/*$(xml).find('prefix').each(function(){
				if($(this).children().size() > 0 ){
						var ltr = $(this).attr('letter');                            
						op1 +="<li data-role='list-divider'>"+ltr+"</li>";                                   
						op2 +="<li data-role='list-divider'>"+ltr+"</li>";                                   
						op3 +="<li data-role='list-divider'>"+ltr+"</li>";                                   
			...
			arumbuDb.query("SELECT * FROM names LIMIT 20",function(tx,results){
				var len = results.rows.length;				
				//var op1="<ul data-role='listview' class='nameList'>";
				for(var i=0;i<len;i++) {
					var nameobj = results.rows.item(i);
					console.log("Nameobj.name :::::>"+nameobj.name);
					op1 +="<li>"+nameobj.name+"</li>" 
					
				}
				op1+="</ul>";
				op2+="</ul>";
				op3+="</ul>";
				$('#nameListContainerBoth').html(op1); 
				$('#nameListContainerBoy').html(op2); 
				$('#nameListContainerGirl').html(op3); 
				$('#both').page('destroy').page();	
			});
	...
	"phoneGapInit":function() {
		console.log("phoneGapInit********STARTS");
		arumbuDb.initialize();
		arumbu.init();
		console.log("phoneGapInit*******ENDS");		
	},
...
var arumbuDb = {		
		...
		"prepare":function(tx) {		
			if(tx) {
				...
				tx.executeSql("CREATE TABLE IF NOT EXISTS names (id INTEGER AUTOINCEREMENT PRIMARY KEY, name TEXT, meaning TEXT, gender TEXT, prefix TEXT)");
				tx.executeSql("CREATE TABLE IF NOT EXISTS starred (id INTEGER AUTOINCEREMENT PRIMARY KEY, name_id INTEGER)");				
			}else {				
		...
		"dbError":function(err) {
			//db error handling
			console.log("Error Opening Database :::>" + err.code);
		},
		
		"initialize":function() {
			console.log("arumbuDb.initialize********STARTS");
			this.dbObj = window.openDatabase("thamizha_arumbu","1.0","Thamizha's Arumbu",3145728);//3MB	
			this.prepare();
			this.initializeData();
			console.log("arumbuDb.initialize********ENDS");
		},
